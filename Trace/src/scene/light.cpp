...
Vec3d DirectionalLight::shadowAttenuation(const ray& r, const Vec3d& p) const
{
	...
	const Vec3d &dir = getDirection(p);
	Vec3d result(1.0, 1.0, 1.0);
	// push the point outwards a bit so that the ray won't hit itself
	Vec3d point = p + dir * RAY_EPSILON;
	while (!result.iszero())
	{
		isect i;
		...
		result = prod(result, material.kt(i));
		//result = result * i.getMaterial().kt;

		// pushing the point further away from the object to avoid intersection
